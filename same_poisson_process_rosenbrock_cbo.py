# -*- coding: utf-8 -*-
"""Same Poisson process Rosenbrock CBO.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1JL6VDpcM4Mvq3BrKsQEIcGirUqLffgGH
"""
...
class JumpCBO(BaseCBO):
  ...
  def jumps(self,X, i):
    if self.type_rv == "binomial":
      ...
      k = self.poisson_rv()
      if k != 0:
        jps = np.zeros(self.difference(X).shape)
        gamma = 1 
        for _ in range(k):
          jps = jps +  np.random.normal(size = self.difference(X).shape)
        if i*self.h <= 100:
